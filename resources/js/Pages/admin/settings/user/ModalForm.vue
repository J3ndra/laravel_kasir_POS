...
const update = async () => {
    ...
    axios
        .post(route("settings.user.update", { id: props.data.id }), form.value)
        .then((res) => {
            emit("close");
            emit("successSubmit");
            form.value = ref({});
...
const create = async () => {
    isLoading.value = true;
    axios
        .post(route("settings.user.create"), form.value)
        .then((res) => {
            emit("close");
            emit("successSubmit");
            form.value = ref({});
...
<template>
    <VDialog
        ...
        :title="updateAction ? 'Update User' : 'Create User'"
        @opened="openForm"
        @closed="closeForm"
        size="xl"
    >
        ...
        <template v-slot:content>
            <div class="grid grid-cols-2 gap-3">
                ...
                <VInput
                    ...
                    @update:modelValue="formError.email = ''"
                    type="email"
                />
                ...
                <VInput
                    ...
                    :errorMessage="formError.password_confirmation"
                    @update:modelValue="
