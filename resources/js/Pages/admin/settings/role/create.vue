...
const submit = async () => {
    ...
    axios
        .post(route("settings.role.storerole"), form.value)
        .then((res) => {
            discard();
            notify(
                {
            ...
            <section>
                <div class="w-full">
                    ...
                    <div class="text-sm text-slate-500 mb-4">
                        Fill in to give a name the role
                    </div>
                    <VInput
                        ...
                        :errorMessage="formError.role_name"
                        @update:modelValue="formError.role_name = ''"
            ...
            <section>
                <div class="w-full">
                    ...
                    <div class="text-sm text-slate-500">
                        List to set the permissions used for the role
                    </div>
                </div>
                <div
                    class="w-full my-6"
                    v-for="(data, index) in additional.permission_list"
                    :key="index"
                >
                    <p class="uppercase font-bold text-slate-500 text-xl">
                        {{ index.replace(/_/g, " ") }}
                    </p>
                    <div
                        class="w-full flex justify-between my-4 border-b pb-4"
                        v-for="(subGroup, index) in data"
                        :key="index"
                        :class="{
                            'border-b-0 pb-0':
                                Object.keys(data).pop() === index,
                        }"
                    >
                        <div class="max-w-[60%]">
                            <p
                                class="capitalize text-base text-slate-800 font-semibold"
                            >
                                {{ index.replace(/_/g, " ") }}
                            </p>
                            <div class="font-normal text-slate-400 text-sm">
                                (<span
                                    v-for="(permission, index) in subGroup"
                                    :key="index"
                                >
                                    {{ permission.label
                                    }}{{
                                        index + 1 === subGroup.length
                                            ? ""
                                            : ", "
                                    }} </span
                                >)
                        ...
                        <div class="flex">
                            <div class="flex my-auto">
                                <div
                                    class="text-sm font-normal text-slate-400 italic mr-2"
                                >
                                    {{
                                        subGroup.length === 1
                                            ? "Active"
                                ...
                                <div class="form-switch">
                                    <input
                                        type="checkbox"
                                        :id="subGroup[0]['group'] + '_' + index"
                                        class="sr-only"
                                        @change="
                                            handleActiveAll(subGroup, index)
                                        "
                            ...
                            >
                                <VButton
                                    ...
                                    @click="handleManageModal(subGroup)"
                                />
                            </div>
                        </div>
                    </div>
