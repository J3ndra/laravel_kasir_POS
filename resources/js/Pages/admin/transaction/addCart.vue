...
const getProduct = async () => {
    axios
        .get(route("transaction.getproductdata"), {
            params: {
...
const submit = async () => {
    ...
    axios
        .post(route("transaction.addtocart"), form.value)
        .then((res) => {
            emit("successSubmit");
            form.value = ref({});
            notify(
...
<template>
    <div
        class="w-full md:w-1/3 rounded-lg bg-white px-4 py-6 border drop-shadow-sm h-fit"
    >
        <div class="grid grid-cols-1 gap-3">
            ...
            <VSelect
                ...
                label="Product"
                :errorMessage="formError.product_id"
                @update:modelValue="formError.product_id = ''"
                ref="productSelectHandle"
