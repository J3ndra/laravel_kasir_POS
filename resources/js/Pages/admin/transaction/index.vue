...
const getCartData = debounce(async () => {
    axios
        .get(route("transaction.getcartdata"))
        .then((res) => {
...
const deleteProductFromCart = async () => {
    axios
        .delete(
            route("transaction.deletefromcart", { id: itemSelected.value.id })
        )
        .then((res) => {
            notify(
                {
...
const payOrder = async () => {
    axios
        ...
        .then((res) => {
            ...
            setTimeout(() => {
                window.open(
                    route("transaction.printorder", { id: res.data.data.id }),
                    "_blank"
                );
            }, 50);
        })
...
<template>
    ...
    <div class="flex flex-col space-y-3 md:flex-row md:space-x-3 md:space-y-0">
        ...
        <div class="w-full md:w-2/3">
            ...
            <div class="border rounded bg-white drop-shadow-sm p-3 mb-3">
                <div class="flex justify-between">
                    <p class="my-auto">Grand Total</p>
                    <div v-if="cartLoading" class="w-6">
                        <VLoading />
                    </div>
                    <p v-else>Rp{{ number_format(grandTotal, 2, ",", ".") }}</p>
                </div>
                <div class="flex justify-between" v-if="form.cash">
                    <p class="my-auto">Change</p>
                    <div v-if="changeLoading" class="w-6">
                        <VLoading />
                    </div>
                    <p v-else>Rp{{ number_format(change, 2, ",", ".") }}</p>
                </div>
            </div>
            <!-- Carts Table -->
            <div class="border rounded bg-white drop-shadow-sm p-4">
                ...
                <section class="mt-5">
                    ...
                    >
                        ...
                        <tr v-else-if="query.length === 0 && !cartLoading">
                            ...
                            >
                                <div class="flex items-center flex-col w-full">
                                    <div
                                        class="my-2 text-slate-500 text-md font-medium"
                                    >
                                        Result not found.
                                    </div>
                                </div>
                        ...
                        <tr v-for="(data, index) in query" :key="index" v-else>
                            ...
                            <td class="px-4 whitespace-nowrap h-16">
                                Rp{{ data.product.price_formatted }}
                            </td>
                            <td class="px-4 whitespace-nowrap h-16">
                                {{ data.qty }}
                            </td>
                            <td class="px-4 whitespace-nowrap h-16">
                                Rp{{ data.sub_total_formatted }}
                            </td>
                ...
                <section
                    ...
                    v-if="query.length > 0 && !cartLoading"
                >
                    <div class="flex justify-end">
                        <VInput
                            placeholder="Insert Cash (Rp)"
                            label="Pay"
                            :required="true"
                            v-model="form.cash"
                            :errorMessage="formError.cash"
                            @update:modelValue="
                                (formError.cash = ''), calculateChange()
                            "
                            type="number"
                        />
                    </div>
        ...
        <VAlert
            ...
            @submitAlert="deleteProductFromCart"
            type="danger"
